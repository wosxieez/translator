        ...
        if (data != nil)
        {
            ...
            if (resultDic != nil)
            {
                ...
                if (result != nil)
                {
                    ...
                    // 翻译
                    translateResult = ""
                    AppService.getInstance().translate(q: thisResult, from: AppUtil.sourceLanguage.translateCode, to: AppUtil.targetLanguage.translateCode, completionHandler: translateCompletion);
                }
                else
                {
                    thisCallback("无法识别", "");
...
extension SpeechSourceToTarget: XMLParserDelegate {
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        if elementName == "string" {
            thisCallback(thisResult, translateResult)
