    ...
    private func sendData() -> Void
    {
        ...
        var request:URLRequest =
            URLRequest(url: URL(string: "https://speech.platform.bing.com/speech/recognition/interactive/cognitiveservices/v1?language=" +
                AppUtil.targetLanguage.code + "&format=simple")!);
        ...
        if (data != nil)
        {
            ...
            if (resultDic != nil)
            {
                ...
                if (result != nil)
                {
                    thisResult = result;
                    // 翻译
                    AppUtil.translate(q: thisResult, language: AppUtil.sourceLanguage.speechCode, completionHandler: translateCompletion);
                }
                else
                {
                    thisCallback("无法识别", "");
