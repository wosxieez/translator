...
class TranslateView3Controller: UIViewController, AVCapturePhotoCaptureDelegate
{
    ...
    let deviceOutput:AVCapturePhotoOutput = AVCapturePhotoOutput();
    ...
    {
        print("请求权限反馈...\(permission)", Thread.current);
        if (permission)
        {
            // 开始查找捕获设备
    ...
    {
        let photoSetting:AVCapturePhotoSettings = AVCapturePhotoSettings(format: [AVVideoCodecKey: AVVideoCodecJPEG]);
        deviceOutput.capturePhoto(with: photoSetting, delegate: self);
    }
    ...
    func ocrCompletionHandler(data:Data?, response:URLResponse?, error:Error?) -> Void
    {
        ...
        DispatchQueue.main.async {
            ...
            if (resultDic != nil)
            {
                ...
                {
                    self.sourceStr = str;
                    AppUtil.translate2(q: str, completionHandler: self.translateCompletion);
                }
                else
                {
    ...
    func translateCompletion(result:String?) -> Void
    {
        DispatchQueue.main.async {
            ...
            let alert:UIAlertController = UIAlertController(title: "识别结果", message: resultStr,
